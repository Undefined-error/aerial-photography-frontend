...
// 定义emit
// const emit = defineEmits(["refresh"]); // 暂时保留，虽然没有被使用
...
// 从 store 中获取企业ID作为详情查询的参数
const enterpriseId = computed(() => userStore.enterpriseInfo?.id);
...
const handleFileChange = async (event) => {
  ...
  const id = enterpriseId.value;
  if (!id) {
    ElMessage.error("企业ID缺失，无法上传Logo！");
    event.target.value = null;
    return;
  }
...
const getDetail = async () => {
  const id = enterpriseId.value;
  if (!id) {
    ElMessage.warning("无法获取企业ID，请确保用户登录且企业信息已加载。");
    // 尝试用 store 里的基础信息填充，如果 store 有的话
    enterDetailObj.value = userStore.enterpriseInfo || {};
    return;
  }

  try {
    const res = await enterDetail(id);
    if (res && res.code === 200) {
      // 深拷贝确保响应式生效
